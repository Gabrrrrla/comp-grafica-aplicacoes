...
uniform samplerCube skybox; // <-- NOVO: sampler para o cubemap

// Propriedades da superfície/material
uniform float ka;
uniform float kd;
uniform float ks, q;
uniform float reflectivity; // <-- NOVO: controle de mistura (0.0 a 1.0)
...
void main()
{
    // Integração com iluminação local
    ...
    // Cor base (iluminação + textura 2D da Suzanne)
    vec4 texColor = texture(texBuffer, texcoord);
    vec4 corPhong = (vec4(ambient, 1.0) + vec4(diffuse, 1.0)) * texColor;

    // Reflexão de ambiente
    // Vetor de incidência I (da câmera para o fragmento)
    vec3 I = normalize(fragPos - cameraPos); 
    // Vetor de reflexão R
    vec3 R = reflect(I, N);                  
    // Amostra a cor do céu usando R
    vec4 corSkybox = texture(skybox, R);     

    color = mix(corPhong, corSkybox, reflectivity);
}
)glsl";
...
int main()
{
    ...
    glUniform1i(glGetUniformLocation(shaderID, "texBuffer"), 0); // Corrigido para texBuffer

    // Define a unidade de textura 1 para o cubemap no shader do objeto
    glUniform1i(glGetUniformLocation(shaderID, "skybox"), 1);
    
    // Define a intensidade do reflexo
    glUniform1f(glGetUniformLocation(shaderID, "reflectivity"), 0.4f);
	...
	while (!glfwWindowShouldClose(window))
	{
        ...
        // Ativa e vincula a Textura 2D
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, m.texID);

        // Ativa e vincula o Cubemap para os reflexos
